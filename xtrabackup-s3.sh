...
test -z "$PROJECT" || test -z "$BUCKET_PATH" && (printf "Project Name or BUCKET Path is undefined\nUsage: %s [-n] [-p]\n" "$0"; exit)

BACKUP_NAME=$PROJECT-$(date +%F_%H-%M-%S).xbstream
BACKUP_NAME_LAST=$PROJECT-last.xbstream
BUCKET_PATH_FULL=$PROJECT:$BUCKET_PATH/$PROJECT
...
rclone move "$BACKUP_NAME" "$BUCKET_PATH_FULL/"
rclone copyto "$BUCKET_PATH_FULL/$BACKUP_NAME" "$BUCKET_PATH_FULL/$BACKUP_NAME_LAST"